...
const STATE_CONTEXT = {
  ...
  IL:{cities:["Chicago","Aurora","Naperville","Joliet","Rockford"],note:"Illinois is a three-insurer market for TeleDirectMD. Aetna, BCBS of Illinois, and UHC are all in-network, covering the majority of Illinois employer-sponsored plan members."},
  MI:{cities:["Detroit","Grand Rapids","Ann Arbor","Lansing","Flint"],note:"Michigan residents with Aetna commercial plans can access TeleDirectMD's physician-led telemedicine for urgent care and chronic medication refills statewide."},
  ...
  OH:{cities:["Columbus","Cleveland","Cincinnati","Toledo","Akron"],note:"Ohio is a strong two-insurer market with both Aetna and UHC in-network. Ohio residents have broad insurance coverage options for TeleDirectMD telehealth visits."},
...
export default function AetnaStateClient({ stateSlug }) {
  ...
  const FAQS = [
    ...
    { q:`What is the Aetna telehealth copay in ${stateName}?`, a:`Aetna commercial telehealth copays in ${stateName} typically range from $0 to $40. Many employer-sponsored Aetna plans have $0 telehealth copays. Log into your Aetna member portal at aetna.com or call member services to verify your exact cost.` },
    { q:`What conditions does Aetna cover for telemedicine in ${stateName}?`, a:`Aetna commercial plans in ${stateName} cover telehealth visits for a wide range of conditions including UTI, sinus infection, strep throat, ear infection, pink eye, flu, asthma inhaler refills, hypertension medication refills, acid reflux, yeast infections, and many more. Coverage is subject to your specific plan benefits.` },
    { q:`How do I book a TeleDirectMD visit using Aetna in ${stateName}?`, a:`Book at teledirectmd.com/book-online, select "Insurance" as your payment method, and have your Aetna member ID card ready. We verify your coverage before your visit. You can also book as a self-pay patient at any time for a flat $49 fee — no insurance needed.` },
    { q:`What if my specific Aetna plan in ${stateName} is not covered?`, a:`If your specific Aetna plan is not in-network with TeleDirectMD in ${stateName}, you can still see Dr. Bhavsar as a self-pay patient for a flat $49 fee. You may also be eligible to submit an out-of-network claim to Aetna — contact Aetna to learn about your out-of-network benefits.` },
  ];
      ...
      <div style={{ background:`linear-gradient(165deg, ${B.navyDarker} 0%, ${B.navy} 40%, ${B.navyDeep} 100%)`, padding:"56px 24px 64px", position:"relative", overflow:"hidden", marginTop:16 }}>
        ...
        <div style={{ maxWidth:760, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <h1 style={{ fontFamily:B.fd, fontSize:"clamp(28px, 5vw, 44px)", fontWeight:700, color:B.white, lineHeight:1.15, margin:"0 0 16px" }}>
            Online Doctor That Accepts<br />Aetna Insurance in {stateName}
          </h1>
          <p style={{ fontFamily:B.fb, fontSize:"clamp(15px, 2.5vw, 18px)", color:"rgba(255,255,255,0.75)", lineHeight:1.6, margin:"0 0 12px", maxWidth:580 }}>
            TeleDirectMD is in-network with Aetna commercial plans in {stateName}. See a board-certified physician by video — your standard Aetna copay applies.
          </p>
      ...
      <div style={{ maxWidth:880, margin:"0 auto", padding:"0 24px" }}>
        ...
        <section style={{ marginBottom:40 }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:12 }}>
            {[
              {icon:<Ico.Shield c="#7B2CBF" s={22}/>, title:"Aetna In-Network", sub:`Active contract — ${stateName}`},
              {icon:<Ico.Dollar c={B.accent} s={22}/>, title:"Standard Aetna Copay", sub:"$0–$40 most commercial plans"},
              {icon:<Ico.Cal c={B.teal} s={22}/>, title:"Same-Day Visits", sub:"7 days a week availability"},
              {icon:<Ico.Rx c={B.teal} s={22}/>, title:"Rx to Your Pharmacy", sub:"Sent immediately after visit"},
            ].map((fact,i) => (
              <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:B.r, padding:"20px 16px", boxShadow:B.shadow, display:"flex", alignItems:"flex-start", gap:12 }}>
                <div style={{ width:44, height:44, borderRadius:12, background:B.bg, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{fact.icon}</div>
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:B.navy, marginBottom:2 }}>{fact.title}</div>
                  <div style={{ fontSize:13, color:B.text }}>{fact.sub}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <section style={{ marginBottom:48 }}>
          <h2 style={{ fontFamily:B.fd, fontSize:24, fontWeight:700, color:B.navy, margin:"0 0 8px" }}>Conditions Aetna Covers for Telemedicine in {stateName}</h2>
          <p style={{ fontSize:15, color:B.text, margin:"0 0 20px", lineHeight:1.6 }}>Click any condition to see state-specific coverage details for Aetna in {stateName}:</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(220px, 1fr))", gap:10 }}>
            {Object.values(INSURANCE_CONDITIONS).map((cond,i) => (
              <a key={i} href={`/insurance/aetna/${stateSlug}/${cond.slug}`}
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:"#7B2CBF" }}>Aetna + {stateName}</div>
                </div>
              </a>
            ))}
          </div>
        ...
        {ctx.cities.length > 0 && (
          <section style={{ marginBottom:40 }}>
            <h2 style={{ fontFamily:B.fd, fontSize:22, fontWeight:700, color:B.navy, margin:"0 0 8px" }}>Serving {stateName} Aetna Members Statewide</h2>
            <p style={{ fontSize:15, color:B.text, margin:0, lineHeight:1.6 }}>TeleDirectMD serves Aetna members throughout {stateName}, including {ctx.cities.join(", ")}, and all surrounding areas. Since care is delivered by secure video, your location within {stateName} does not affect your ability to book a same-day visit.</p>
          </section>
        )}

        <section style={{ marginBottom:48 }}>
          <h2 style={{ fontFamily:B.fd, fontSize:24, fontWeight:700, color:B.navy, margin:"0 0 8px" }}>How to Use Your Aetna Benefits in {stateName}</h2>
        ...
        <div style={{ marginBottom:24, display:"flex", flexWrap:"wrap", gap:10 }}>
          ...
          <span style={{ color:B.border }}>|</span>
          <a href={`/${stateSlug}/online-doctor-visits`} style={{ fontSize:14, color:B.teal, textDecoration:"none" }}>{stateName} Online Doctor Visits</a>
        </div>
