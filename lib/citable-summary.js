...
// Used by: app/[slug]/[conditionSlug]/page.js  (2,520 pages: 60 conditions × 43 states)
...
export function summarizeStateLanding({ state }) {
  ...
  const answerHtml =
    ...
    `${DOCTOR} (NPI: ${NPI}) is a board-certified Family Medicine physician licensed in ${stateName} and 41 other US states. ` +
...
export function summarizeNationalCondition({ condition }) {
  ...
  const clinSentence = clinicalAppropriatenessSentence({ conditionType, condName, guidelineSrc });
  const answerHtml =
    `Yes \u2014 TeleDirectMD provides same-day online physician visits for ${lc}${icd} to adults in 43 US states for $79 cash-pay, no insurance required. ` +
...
export function summarizeInsurerHub({ insurerName, stateCount, stateList }) {
  ...
  const answerHtml =
    ...
    `Self-pay alternative: $79 flat (FSA/HSA eligible). Per ${SRC.pennMed}, telehealth visits cost roughly five times less than equivalent in-person primary care ($96 vs $509 mean).`;
  return { question, answerHtml, answerText: stripHtml(answerHtml) };
}
...
export function summarizeSymptom({ symptomName, possibleConditions }) {
  ...
  const answerHtml =
    ...
    `TeleDirectMD provides same-day adult video visits with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 43 US states, for $79 cash-pay or in-network with Aetna, Blue Cross Blue Shield, and United Healthcare commercial plans in participating states. ` +
...
export function summarizeCompare({ competitorName, tdmdPrice = 49, competitorPriceRange, competitorCoverageStates, tdmdCoverageStates = 42 }) {
  const compRange = competitorPriceRange || 'higher than TeleDirectMD\u2019s flat $79';
...
export function summarizeUseCase({ useCaseTitle, useCaseDescription, price = 49 }) {
  ...
  const answerHtml =
    `Book a same-day $${price} cash-pay telehealth visit at TeleDirectMD with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 43 US states. ` +
...
export function summarizeWhoWeServe({ segmentName, segmentDescription }) {
  ...
  const answerHtml =
    ...
    `$79 cash-pay (FSA/HSA eligible) gets a same-day video visit with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 43 US states.${desc} ` +
...
export function summarizeHomepage() {
  ...
  const answerHtml =
    `TeleDirectMD is a physician-founded, MD-only telehealth platform providing same-day video visits with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 43 US states + Washington DC. ` +
...
export function summarizeInsuranceHub() {
  ...
  const answerHtml =
    ...
    `Medicaid, Medicare Advantage, HMO, and QPOS plans are not currently in-network. Patients with non-contracted plans (or any plan, in any state) can book a self-pay visit for $79 flat \u2014 FSA/HSA eligible. ` +
    `Per ${SRC.pennMed}, telehealth visits cost roughly five times less than equivalent in-person primary-care visits ($96 vs $509 mean).`;
  return { question, answerHtml, answerText: stripHtml(answerHtml) };
}
...
export function summarizeAbout() {
  ...
  const answerHtml =
    ...
    `He is licensed in 43 US states + Washington DC, holds in-network telehealth contracts with Aetna, Blue Cross Blue Shield, and United Healthcare commercial plans, and personally conducts every TeleDirectMD video visit. ` +
...
export function summarizeFaqLanding() {
  ...
  const answerHtml =
    `A TeleDirectMD telehealth visit is a video appointment with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 43 US states + DC. ` +
