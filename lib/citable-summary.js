...
export function summarizeConditionState({ state, condition }) {
  ...
  // Prefer the clean conditionName field (present on all 60 condition JSONs).
  // Fall back through displayName/shortName/h1 (future-proofing), and finally
  // derive a clean Title Case name from the slug (e.g. 'epipen-refills-online'
  // -> 'EpiPen Refills'). Last resort: a generic phrase.
  // NEVER fall back to pageTitle: it is a pipe-stuffed SEO title that produced
  // doorway-page-style H2/QAPage output and caused mass deindexing in May 2026.
  const slugDerived = (() => {
  ...
  const condName =
    ...
    slugDerived ||
    'this condition';
  // Avoid duplicate 'treatment' when conditionName already ends in Treatment/Refills/etc.
  const lc = condName.toLowerCase();
  const alreadyHasService = /(treatment|refills?|prevention|cessation)$/i.test(lc.trim());
  const serviceSuffix = alreadyHasService ? '' : ' treatment';
  ...
  const question = `How can I get ${lc}${serviceSuffix} online in ${stateName}?`;
  const answerHtml =
    `TeleDirectMD provides board-certified physician video visits for ${lc}${icd} to adult residents of ${stateName}. ` +
...
// MedicalGuideline catalog — maps citation domain substrings to a structured
// MedicalGuideline schema node. When a citableSummary cites one of these
// authorities (via inline <a href>), we emit a MedicalGuideline graph entry
// so AI extractors see the citation as a graded medical evidence source.
const MEDICAL_GUIDELINES = [
  ...
  { substr: 'aad.org',                       name: 'AAD Clinical Guidelines (2024)',                  url: 'https://www.aad.org/member/clinical-quality/guidelines', publisher: 'American Academy of Dermatology' },
  ...
  { substr: 'aan.com',                       name: 'AAN Practice Guidelines (2024)',                  url: 'https://www.aan.com/practice/guidelines',        publisher: 'American Academy of Neurology' },
  { substr: 'professional.diabetes.org',     name: 'ADA Standards of Care in Diabetes (2026)',         url: 'https://professional.diabetes.org/standards-of-care', publisher: 'American Diabetes Association' },
  { substr: 'acc.org',                       name: 'ACC/AHA Cardiovascular Practice Guidelines (2024)', url: 'https://www.acc.org/Guidelines',                 publisher: 'American College of Cardiology / American Heart Association' },
  { substr: 'aao.org',                       name: 'AAO Preferred Practice Pattern: Conjunctivitis',   url: 'https://www.aao.org/clinical-statement/conjunctivitis-summary-benchmark', publisher: 'American Academy of Ophthalmology' },
];
...
export function citableSummaryToJsonLd(summary, { pageUrl, datePublished = TODAY } = {}) {
  ...
  if (guidelines.length === 0) {
    return { '@context': 'https://schema.org', ...qaPage };
  }
  return {
    '@context': 'https://schema.org',
    '@graph': [qaPage, ...guidelines],
  };
}
