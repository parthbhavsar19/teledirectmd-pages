...
export function summarizeUseCase({ useCaseTitle, useCaseDescription, price = 79 }) {
  ...
  const answerHtml =
    `Book a same-day $${price} cash-pay telehealth visit at TeleDirectMD with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 40+ states + DC. ` +
...
export function summarizeWhoWeServe({ segmentName, segmentDescription }) {
  ...
  const answerHtml =
    ...
    `$79 cash-pay (FSA/HSA eligible) gets a same-day video visit with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 40+ states and Washington, D.C.${desc} ` +
...
export function summarizeInsuranceHub() {
  ...
  const answerHtml =
    `TeleDirectMD is in-network as a telehealth provider with three major commercial insurance carriers across 17 states: <strong>Aetna</strong> (AZ, CA, CO, FL, GA, IL, LA, MI, MN, NC, OH, PA, TN, WA), <strong>Blue Cross Blue Shield affiliates</strong> (BCBS-AZ in AZ, Florida Blue in FL, Anthem in GA, BCBS-IL in IL, BCBS-MI in MI, Highmark in PA, BCBS-TX in TX), and <strong>UnitedHealthcare</strong> (AZ, CA, CO, FL, GA, IL, LA, MI, MN, NC, NJ, OH, OK, PA, TN, TX, WA). ` +
...
export function summarizeWhatWeTreat() {
  ...
  const answerHtml =
    ...
    `Reproductive health: STI testing/treatment (chlamydia, trichomoniasis, herpes), DoxyPEP STI prevention, BV, yeast, vaginal dryness. ` +
