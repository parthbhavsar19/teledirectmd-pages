...
export function summarizeSymptom({ symptomName, possibleConditions }) {
  ...
  const answerHtml =
    ...
    `TeleDirectMD provides same-day adult video visits with ${DOCTOR} (NPI: ${NPI}), a board-certified hospitalist physician licensed in 42 US states, for $79 cash-pay or in-network with Aetna, Blue Cross Blue Shield, and United Healthcare commercial plans in participating states. ` +
...
export function summarizeWhoWeServe({ segmentName, segmentDescription }) {
  ...
  const answerHtml =
    ...
    `$79 cash-pay (FSA/HSA eligible) gets a same-day video visit with ${DOCTOR} (NPI: ${NPI}), a board-certified hospitalist physician licensed in 42 US states.${desc} ` +
