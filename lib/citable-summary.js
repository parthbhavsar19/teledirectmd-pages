...
export function summarizeNationalCondition({ condition }) {
  ...
  const clinSentence = clinicalAppropriatenessSentence({ conditionType, condName, guidelineSrc });
  const answerHtml =
    `Yes \u2014 TeleDirectMD provides same-day online physician visits for ${lc}${icd} to adults in 42 US states for $79 cash-pay, no insurance required. ` +
...
export function summarizeSymptom({ symptomName, possibleConditions }) {
  ...
  const answerHtml =
    ...
    `TeleDirectMD provides same-day adult video visits with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 42 US states, for $79 cash-pay or in-network with Aetna, Blue Cross Blue Shield, and United Healthcare commercial plans in participating states. ` +
...
export function summarizeUseCase({ useCaseTitle, useCaseDescription, price = 49 }) {
  ...
  const answerHtml =
    `Book a same-day $${price} cash-pay telehealth visit at TeleDirectMD with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 42 US states. ` +
...
export function summarizeWhoWeServe({ segmentName, segmentDescription }) {
  ...
  const answerHtml =
    ...
    `$79 cash-pay (FSA/HSA eligible) gets a same-day video visit with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 42 US states.${desc} ` +
...
export function summarizeHomepage() {
  ...
  const answerHtml =
    `TeleDirectMD is a physician-founded, MD-only telehealth platform providing same-day video visits with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 42 US states + Washington DC. ` +
...
export function summarizeFaqLanding() {
  ...
  const answerHtml =
    `A TeleDirectMD telehealth visit is a video appointment with ${DOCTOR} (NPI: ${NPI}), a board-certified Family Medicine physician licensed in 42 US states + DC. ` +
