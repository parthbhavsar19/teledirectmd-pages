...
// 2-letter abbreviations for the 44 licensed states + D.C..
const STATE_ABBRS = ['AL','AK','AZ','CA','CO','CT','DC','DE','FL','GA','HI','IA','ID','IL','IN','KS','KY','LA','MD','ME','MI','MN','MO','MS','MT','NC','ND','NE','NH','NJ','NV','OH','OK','PA','SC','SD','TN','TX','UT','VA','VT','WA','WI','WV','WY'];
 ...
 *                                        standard 3-step "book a $79 visit" flow
 ...
 *        defaults to the $79 telehealth visit Offer.
...
export function buildCostCompareJsonLd({
  ...
  const defaultHowTo = {
    ...
    step: [
      { '@type': 'HowToStep', position: 1, name: 'Book online',          text: 'Book a same-day visit at teledirectmd.com/book-online for $79 flat. No insurance required.' },
  ...
  // Default Service+Offer node (the $79 telehealth visit). Pages can override
  // with their own service config (e.g. travel-prescription Service).
  const serviceNode = {
    '@type': 'Service',
    '@id': `${pageUrl}#service`,
    name: (service && service.name) || 'TeleDirectMD $79 Telehealth Visit',
    serviceType: (service && service.serviceType) || 'Telehealth medical consultation',
    provider: { '@id': `${BASE_URL}#org` },
    areaServed: STATE_ABBRS,
    offers: {
      '@type': 'Offer',
      price: (service && service.price) || '79',
  ...
  if (costPlus && costPlus.drug) {
    graph.push({
      '@type': 'Pharmacy',
      '@id': `${pageUrl}#cost-plus-drugs`,
      name: 'Mark Cuban Cost Plus Drug Company',
      url: 'https://costplusdrugs.com/',
      makesOffer: {
        '@type': 'Offer',
        itemOffered: { '@type': 'Drug', name: costPlus.drug },
        price: costPlus.price || undefined,
        priceCurrency: 'USD',
        description: costPlus.note,
      },
    });
  }
...
/**
 * AggregateOffer schema for cost-comparison pages that have a provider-by-provider
 * price table. Each entry becomes an Offer with Organization seller, price range,
 * and area served. Returns null if cfg.costComparisonOffers is absent — only
 * opted-in cost slugs emit this schema.
 *
 * AI engines (Perplexity, ChatGPT) preferentially cite structured AggregateOffer
 * blocks when answering "how much does X cost" / cheapest-X queries.
 */
export function buildCostComparisonAggregateOffer({ pageUrl, headline, costComparisonOffers, conditionLabel }) {
  if (!costComparisonOffers || !Array.isArray(costComparisonOffers) || costComparisonOffers.length === 0) {
    return null;
  }
  return {
    '@context': 'https://schema.org',
    ...
    name: `${conditionLabel} cost comparison`,
    description: headline,
    url: pageUrl,
    priceCurrency: 'USD',
    lowPrice: Math.min(...costComparisonOffers.map(o => o.lowPrice)),
    highPrice: Math.max(...costComparisonOffers.map(o => o.highPrice)),
    offerCount: costComparisonOffers.length,
    offers: costComparisonOffers.map(o => ({
      '@type': 'Offer',
      name: o.providerName,
      seller: {
        '@type': 'Organization',
        name: o.providerName,
        ...(o.providerUrl ? { url: o.providerUrl } : {}),
      },
      priceCurrency: 'USD',
