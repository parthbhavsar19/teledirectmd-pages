...
/* This page is a committed light design, so keep the document ground white even
   in dark mode. Otherwise the near-black body background shows through the
   footer's top margin and on overscroll as a hard band against the white page. */
body:has(.uac-wrap){background:#fff !important;}

/* Forcing that ground white is only half the job. The shared .tdmd-* modules that
   sit directly on it — the prescribing-limits notice, the pricing CTA, the
   prescribing FAQ — still take their colours from :root, which is in dark mode,
   so they rendered near-white on the new white ground (1.19:1, invisible).
   Pin the tokens back to their light values for this page's document, and
   override FaqAccordion's dark rules, which hardcode colours off :root rather
   than reading a token. */
body:has(.uac-wrap){
  --tdmd-teal:#006b73;
  --tdmd-navy:#003e52;
  --tdmd-accent:#ff5a36;
...
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__title,
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__question{color:#143938 !important;}
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__item{background:#FFF !important;border-color:#E5E0DB !important;}
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__toggle{background:#F5F2EE !important;border-color:#E5E0DB !important;color:#1B4D4C !important;}
body:has(.uac-wrap) .tdmd-faq-accordion details[open] > .tdmd-faq-accordion__question .tdmd-faq-accordion__toggle{background:#1B4D4C !important;color:#FFF !important;border-color:#1B4D4C !important;}
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__answer{color:#3d5350 !important;}
body:has(.uac-wrap) .tdmd-faq-accordion .tdmd-faq-accordion__answer strong{color:#143938 !important;}
...
.uac-wrap .uac-btn--glass{background:rgba(255,255,255,0.12);color:#fff;}
.uac-wrap .uac-btn--glass:hover{background:rgba(255,255,255,0.2);color:#fff;}
...
/* ── Hero (video) ──────────────────────────────────────────────────────── */
.uac-hero{
  position:relative;background:var(--uac-ink-deep);color:#fff;overflow:hidden;
  min-height:640px;display:flex;align-items:center;
}
.uac-hero-video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;background:var(--uac-ink-deep);}
.uac-hero-overlay{
  position:absolute;inset:0;
  background:linear-gradient(90deg, rgba(14,42,56,0.82) 0%, rgba(14,42,56,0.62) 45%, rgba(14,42,56,0.12) 100%);
}
.uac-hero-container{position:relative;z-index:1;max-width:1200px;width:100%;margin:0 auto;padding:48px 24px 128px;}
.uac-crumbs{display:flex;flex-wrap:wrap;gap:8px;font-size:13px;color:var(--uac-muted-dark);margin-bottom:40px;}
.uac-crumbs a{color:#fff;}
.uac-crumbs a:hover{color:var(--uac-orange);}
.uac-crumbs [aria-current]{color:var(--uac-muted-dark);}
.uac-hero-copy{display:flex;flex-direction:column;gap:28px;max-width:560px;animation:uacRise 400ms var(--uac-ease-out) both;}
.uac-hero-copy h1{font-size:clamp(42px,5.2vw,76px);line-height:1.02;letter-spacing:-0.025em;color:#fff;}
.uac-hero-sub{font-size:19px;color:#fff;max-width:46ch;}
...
.uac-hero .uac-chiprow-label{color:var(--uac-muted-dark);}
...
.uac-hero .uac-chip{border-color:rgba(255,255,255,0.35);color:#fff;}
.uac-wrap .uac-chip:hover{border-color:var(--uac-orange);}
.uac-hero .uac-chip:hover{color:#fff;border-color:var(--uac-orange);}
.uac-byline{font-size:13px;color:var(--uac-muted-dark);}
.uac-byline a{text-decoration:underline;color:#fff;}
.uac-byline a:hover{color:var(--uac-orange);}
...
.uac-overlap-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:24px;align-items:start;}
.uac-pricecard{
  background:#FFFFFF;color:var(--uac-ink);border-radius:28px;padding:40px;
  border:1px solid var(--uac-border);box-shadow:var(--uac-shadow-lg);
...
.uac-price-note{font-family:var(--uac-mono);font-size:13px;color:var(--uac-muted-light);line-height:1.45;}
.uac-pricecard ul{
  list-style:none;margin:0;padding:20px 0 0;
  border-top:1px solid var(--uac-border);
  display:flex;flex-direction:column;gap:12px;font-size:15px;
}
.uac-pricecard li{display:flex;gap:12px;align-items:center;}
.uac-pricecard-foot{font-size:12px;color:var(--uac-muted-light);text-align:center;}

.uac-doctorcard{
  background:#FFFFFF;color:var(--uac-ink);border:1px solid var(--uac-border);border-radius:28px;
  box-shadow:var(--uac-shadow-lg);padding:32px 28px;
  display:flex;flex-direction:column;gap:20px;
}
.uac-doctorcard-img{width:96px;height:96px;border-radius:999px;object-fit:cover;object-position:top;flex:none;}
.uac-doctorcard-body{display:flex;flex-direction:column;gap:4px;min-width:0;}
.uac-doctorcard-name{font-weight:600;}
.uac-doctorcard-name span{font-weight:400;color:var(--uac-muted-light);}
.uac-doctorcard-quote{font-size:15px;color:var(--uac-muted-light);}
.uac-doctorcard-link{font-size:14px;font-weight:600;text-decoration:underline;flex:none;}

.uac-examplevisit{
  display:flex;flex-wrap:wrap;align-items:center;gap:10px 16px;
  padding:14px 18px;border-radius:14px;background:var(--uac-soft);border:1px solid var(--uac-border);
  font-size:14px;color:var(--uac-ink);
}
.uac-examplevisit-label{font-family:var(--uac-mono);font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:var(--uac-orange);}
.uac-examplevisit-note{font-size:11px;color:var(--uac-muted-light);margin-left:auto;}

/* ── Comparison (dark) ─────────────────────────────────────────────────── */
.uac-compare{background:var(--uac-ink-deep);color:#fff;padding:96px 24px;margin-top:96px;}
...
@media (max-width:900px){
  ...
  .uac-hero{min-height:520px;}
  .uac-hero-container{padding:32px 20px 96px;}
  .uac-overlap-grid{gap:20px;}
  .uac-pricecard{padding:32px;}
  .uac-price-big{font-size:72px;}
  .uac-compare,.uac-doctor,.uac-conditions{padding:64px 20px;}
  .uac-compare{margin-top:64px;}
  ...
  .uac-doctorcard{padding:24px 20px;}
}
