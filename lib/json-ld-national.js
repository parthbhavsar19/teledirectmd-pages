...
export function generateNationalJsonLd(condition) {
  ...
  graph.push({
    ...
    "offers": {
      ...
      "price": "79",
      "priceCurrency": "USD",
      "description": "Transparent self pay option starting at $79 for an adult video visit. Insurance is not required but select plans are accepted in some states.",
      "eligibleRegion": "US",
  ...
  // FAQPage — direct-answer Q&A (if any) are surfaced first so AI extractors
  // see the high-intent answers at the top of the FAQ entity. They render
  // visibly in the page's direct-answer block, so schema parity is preserved.
  const directAnswerItems = condition.nationalOverrides?.directAnswer?.items || [];
  const faqItems = [...directAnswerItems, ...(condition.faq?.items || [])];
  ...
  graph.push({
    ...
    "description": `Book a same-day video visit with a licensed MD for ${condition.conditionName} — available in 40+ states, no waiting room, starting at $79.`,
    "totalTime": "PT15M",
