...
export function generateJsonLd(condition, state) {
  ...
  graph.push({
    ...
    "offers": {
      ...
      "price": "49",
      "priceCurrency": "USD",
      "description": `Transparent self pay option starting at $49 for an adult video visit in ${state.name}. Insurance is not required but select plans are accepted in some states.`,
  ...
  graph.push({
    ...
    "description": `Book a same-day video visit with a licensed MD in ${state.name} for ${condition.conditionName} — no waiting room, starting at $49.`,
    "totalTime": "PT15M",
