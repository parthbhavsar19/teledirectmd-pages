...
export function getTripleMatrixUrl(insurerSlug, stateAbbr, nationalConditionSlug) {
  ...
  if (insurer.conditionMatrix === false) return null;
...
// Secondary cost pages: condition → additional niche cost/answer pages that
// target a distinct high-volume query cluster (e.g. drug/prescription/OTC).
// Surfaced as extra links in the "What does this cost?" block so the full
// cluster is interlinked. Keep aligned with COST_PAGE_SLUGS.
const CONDITION_TO_SECONDARY_COST_PAGES = {
  'uti-treatment-online': [
    { slug: 'uti-antibiotics-cost', title: 'UTI antibiotics: cost, options & how to get them online' },
  ],
};

// Returns an array of { url, title } secondary cost pages for a condition (may be empty).
export function getSecondaryCostPagesForCondition(conditionSlug) {
  const list = CONDITION_TO_SECONDARY_COST_PAGES[conditionSlug] || [];
  return list.map((p) => ({ url: `/cost/${p.slug}/`, title: p.title }));
}
...
export const COMPARE_PAGES_INDEX = [
  { slug: 'teledirectmd-vs-teladoc',           competitor: 'Teladoc',          tagline: '$79 flat vs. Teladoc $89/visit; named MD vs. mixed MD/NP/PA panel.' },
  { slug: 'teledirectmd-vs-mdlive',            competitor: 'MDLIVE',           tagline: '$79 flat vs. MDLIVE $85; 44 states vs. 50 + Puerto Rico.' },
  { slug: 'teledirectmd-vs-amwell',            competitor: 'Amwell',           tagline: '$79 flat vs. Amwell $79; Amwell has 55+ insurance plans.' },
  { slug: 'teledirectmd-vs-doctor-on-demand',  competitor: 'Doctor on Demand', tagline: '$79 flat vs. $99/visit; Doctor on Demand accepts Medicare Part B.' },
  { slug: 'teledirectmd-vs-sesame',            competitor: 'Sesame Care',      tagline: 'Predictable $79 flat vs. provider-set $29–$37+; Sesame takes no insurance.' },
  { slug: 'teledirectmd-vs-k-health',          competitor: 'K Health',         tagline: 'Live MD video at $79 vs. K Health text-based AI triage from $35.' },
];
...
// ─── Symptom-page back-links (UPDATED 2026-07-18) ────────────────────
// The standalone /symptoms/* URLs were retired to eliminate signal
// cannibalization with /cost/* and /health-guides/* pages. The
// "Common Symptoms Patients Ask About" module still renders on condition,
// state×condition, and cost pages — but each symptom card now links
// DIRECTLY to the final redirect target instead of hopping through a 301.
import { CONDITION_TO_SYMPTOMS, SYMPTOM_PAGES } from './symptom-pages-config';

// Post-teardown destinations. Must match the 301 map in vercel.json for the
// /symptoms/{slug}/ routes. If either side changes, update both.
export const SYMPTOM_SLUG_TO_DESTINATION = {
  'acid-reflux':         '/health-guides/acid-reflux-gerd-guide/',
  'acne-breakout':       '/cost/acne-treatment-cost/',
  'allergy-symptoms':    '/health-guides/seasonal-allergy-guidelines-2026/',
  'bronchitis':          '/health-guides/complete-telehealth-conditions-guide/',
  'burning-urination':   '/uti-treatment-online/',
  'ear-pain':            '/cost/ear-infection-cost/',
  'migraine':            '/cost/migraine-treatment-cost/',
  'persistent-cough':    '/what-we-treat/',
  'sinus-pressure':      '/cost/sinus-infection-treatment-cost/',
  'skin-rash':           '/what-we-treat/',
  'sore-throat':         '/cost/sore-throat-treatment-cost/',
  // Slugs that already had 301s in vercel.json before the 2026-07-18 teardown:
  'asthma':              '/asthma-refills-online/',
  'birth-control':       '/birth-control-refills-online/',
  'cold-sore':           '/cold-sore-treatment-online/',
  'cystic-acne':         '/cost/acne-treatment-cost/',
  'erectile-dysfunction':'/erectile-dysfunction-treatment-online/',
  'pink-eye':            '/pink-eye-treatment-online/',
  'sinus-infection':     '/sinus-infection-treatment-online/',
  'strep-throat':        '/cost/sore-throat-treatment-cost/',
  'yeast-infection':     '/yeast-infection-treatment-online/',
};

export function getSymptomsForCondition(conditionSlug) {
  const slugs = CONDITION_TO_SYMPTOMS[conditionSlug] || [];
  return slugs.map((s) => {
    const cfg = SYMPTOM_PAGES[s];
    if (!cfg) return null;
    const dest = SYMPTOM_SLUG_TO_DESTINATION[s];
    if (!dest) return null; // symptom slug not mapped — skip rather than 404
    return {
      slug: s,
      url: dest,
      title: cfg.breadcrumb,
      query: cfg.query,
    };
  }).filter(Boolean);
}
